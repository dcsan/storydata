...
function makeMov() {
  inputDir="500"
	cat ${inputDir}/*.jpg | ffmpeg -y -framerate 1 -f image2pipe -i - grabbed.mp4
}
...
function makeGif() {
  ...
  ffmpeg -y -i grabbed.mp4 -vf "fps=1,scale=640:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop 1 grabbed.gif
